...
def digests_from_consumers(edges):
    """
    {producer: [{file, digest}]} for every output some consumer hashed. The
    store hashes what a job read, never what it wrote. Consumers that
    disagree read different bytes under one name, and that output stays
    undigested.
    """
    seen = {}
    for edge in edges:
...
class Snakemake(Extractor):
    name = "snakemake"
    description = "Snakemake's metadata store"
    ...
    def extract(self, args):
        ...
        return extract(records, workdir)
