...
from clew.provider.snakemake import adapter_paths as dom
...
class EntryNodes(unittest.TestCase):
    ...
    def test_a_sample_enters_at_its_own_trim_align_and_stats(self):
        entry = dom.Snakemake().triggers['sample'].entries(self.graph, {s: [] for s in SAMPLES})
    ...
    def test_sample_1_does_not_swallow_sample_10(self):
        ...
        entry = dom.Snakemake().triggers['sample'].entries(self.graph, {"sample_1": [], "sample_10": []})
...
class Samplesheet(unittest.TestCase):
    def test_the_sample_column_is_read_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(dom.Snakemake().triggers['sample'].ids(sheet), {"sample_1": [], "sample_9": []})
