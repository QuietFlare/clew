...
from clew.provider.nextflow import adapter_viralrecon as viralrecon
...
class TestViralreconRun(unittest.TestCase):
    ...
    def test_primer_scheme_is_load_bearing(self):
        subjects = core_graph.external_input_entry_nodes(
    ...
    def test_ivar_defect_radius(self):
        subjects = core_graph.container_entry_nodes(self.graph, "ivar")
        radius = core.blast_radius(self.graph, subjects)
    ...
    def test_every_load_bearing_input_present(self):
        ...
        for name in viralrecon.Viralrecon().load_bearing_inputs:
