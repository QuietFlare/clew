...
from clew.provider.nextflow import adapter_viralrecon as viralrecon  # sample-keyed adapter; the crate has no donors
...
class TestRoCrateAdapter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        kind = viralrecon.Viralrecon().triggers['sample']
    ...
    def test_aggregator_is_shared(self):
        for sample, r in self.radius.items():
            ...
            (shared,) = r["shared"]
            self.assertEqual(core_graph.describe(self.graph, shared),
                             "MERGE_REPORT")

    def test_shared_reference_reaches_everything(self):
        subjects = core_graph.external_input_entry_nodes(self.graph, "ref.fa")
    ...
    def test_missing_script_fails_closed(self):
        ...
        facts = contribution.classify(self.graph, task_hash, exclusive=False)
