...
from clew.provider.nextflow import adapter_rnaseq as rnaseq
...
class TestRnaseqRun(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        kind = rnaseq.Rnaseq().triggers['sample']
        cls.subjects = kind.ids(SHEET)
        cls.entry = kind.entries(cls.graph, cls.subjects)
    ...
    def test_gtf_shallow_entry_deep_reach(self):
        subjects = core_graph.external_input_entry_nodes(self.graph, "genes.gtf")
    ...
    def test_genome_radius(self):
        subjects = core_graph.external_input_entry_nodes(self.graph, "genome.fa")
        radius = core.blast_radius(self.graph, subjects)
    ...
    def test_aligner_vs_quantifier_asymmetry(self):
        ...
        # contained. Same trigger type, order-of-magnitude different radius, # the reason per-tool blast radii matter.
        star = core.blast_radius(self.graph,
                                 core_graph.container_entry_nodes(self.graph, "star"))
        salmon = core.blast_radius(self.graph,
                                   core_graph.container_entry_nodes(self.graph, "salmon"))
    ...
    def test_every_load_bearing_input_present(self):
        ...
        for name in rnaseq.Rnaseq().load_bearing_inputs:
