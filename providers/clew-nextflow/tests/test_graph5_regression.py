...
class TestFiveDonorRun(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        kind = sarek.Sarek().triggers['patient']
        cls.donors = kind.ids(SHEET)
        cls.entry = kind.entries(cls.graph, cls.donors)
    ...
    def test_withdrawal_radius_per_donor(self):
        for donor, r in self.radius.items():
            ...
            (shared,) = r["shared"]
            self.assertEqual(core_graph.describe(self.graph, shared), "MULTIQC")
    ...
    def test_aggregator_sees_every_donor(self):
        ...
        for donor, r in self.radius.items():
            self.assertTrue(
                any(core_graph.describe(self.graph, h) == "MULTIQC"
                    for h in r["affected"]),
    ...
    def test_a_versioned_needle_reaches_every_samtools_task(self):
        ...
        self.assertTrue(all("_samtools" in self.graph["tasks"][h]["container"]
                            for h in name_only))
        subjects = core_graph.container_entry_nodes(self.graph, "samtools:1.21")
        radius = core.blast_radius(self.graph, subjects)
    ...
    def test_donor_reference_asymmetry(self):
        ...
        ref_entry = set(
            core_graph.external_input_entry_nodes(self.graph, "genome.fasta")["input:genome.fasta"]
        )
        for donor, r in self.radius.items():
