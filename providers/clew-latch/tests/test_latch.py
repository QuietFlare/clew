...
class Extraction(unittest.TestCase):
    ...
    def test_price_prefers_cost_then_price(self):
        self.assertEqual(self.graph["tasks"][ALIGN]["metrics"]["price"], 0.12)
        self.assertEqual(self.graph["tasks"][CALL]["metrics"]["price"], 0.30)
        self.assertNotIn("price", self.graph["tasks"][REPORT].get("metrics", {}))

    def test_duration_from_timestamps(self):
        self.assertEqual(self.graph["tasks"][CALL]["metrics"]["duration_s"], 1800)
        self.assertEqual(self.graph["tasks"][REPORT]["metrics"]["duration_s"], 60)
