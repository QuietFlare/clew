...
class Cromwell(Extractor):
    name = "cromwell"
    description = "Cromwell workflow metadata, for WDL and Terra"
    ...
    def extract(self, args):
        if args.metadata:
            self.metadata = load_metadata(args.metadata)
        else:
            if not args.workflow:
                print("clew: --server needs --workflow <id>", file=sys.stderr)
                raise SystemExit(2)
            self.metadata = fetch(args.server, args.workflow, args.token)
        return extract(self.metadata)
