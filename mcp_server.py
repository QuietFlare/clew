...
def tool_subject_history(store, args):
    ...
    return _with(query.subject_history(entries, args["subject"]),
                 conflicts)


def tool_policy_history(store, _args):
    _, entries, conflicts = store
    return _with(query.policy_history(entries), conflicts)


def tool_policy_in_force(store, args):
    _, entries, conflicts = store
    return _with(query.policy_in_force(entries, args["as_of"]),
                 conflicts)
