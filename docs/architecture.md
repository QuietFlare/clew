...
plan is the record while a page is a view, extractors translate while
core never interprets an engine string, a bundle verifies against
something it does not control, and an effective date is an instant. A
change to any of them is a new ADR that supersedes the old, not an edit.
